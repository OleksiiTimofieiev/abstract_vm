...
bool Lexer::regex_checks(std::string str, std::vector<std::vector<std::string> > &_command_list)
{
	...
	const 			std::regex	commands_with_arguments_int("^(push|assert|more) (int8|int16|int32)\\(([\\-]?[0-9]+)\\)$");
	const 			std::regex	commands_with_arguments_float_double("^(push|assert|more) (float|double)\\(([\\-]?[0-9]+\\.[0-9]+)\\)$");
